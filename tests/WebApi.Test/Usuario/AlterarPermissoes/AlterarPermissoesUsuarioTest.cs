...
namespace Integracao.Test.Usuario.AlterarPermissoes;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class AlterarPermissoesUsuarioTest : FfkApiClassFixture
{
    private readonly string _baseUrl = "usuario/permissoes";
    ...
    private readonly FfkApi.Domain.Entities.Usuario _usuarioRemoverPermissoes;
    private readonly FfkApi.Domain.Entities.Usuario _usuarioGanharPermissoes1;
    private readonly FfkApi.Domain.Entities.Usuario _usuarioGanharPermissoes2;

    public AlterarPermissoesUsuarioTest()
    {
        ...
        _usuarioRemoverPermissoes = (FfkApi.Domain.Entities.Usuario)entidades["UsuarioRemoverPermissoes"];
        _usuarioGanharPermissoes1 = (FfkApi.Domain.Entities.Usuario)entidades["UsuarioGanharPermissoes1"];
        _usuarioGanharPermissoes2 = (FfkApi.Domain.Entities.Usuario)entidades["UsuarioGanharPermissoes2"];
    }
    ...
    public async Task Sucesso_Administrador_Com_Perfis_E_Permissoes()
    {
        ...
        {
            Id = _usuarioGanharPermissoes1.Id.ToString(),
            PerfisAcesso = [_perfilAcessoGerente.Nome],
            Permissoes = [_permissaoCadastroEquipes.Nome]
        };

        var response = await HttpHelper.DoPut(url: _baseUrl, cancellationToken: cancellationToken, request: request, token: token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NoContent));
    }

    public async Task Sucesso_Administrador_Sem_Perfis_Nem_Permissoes()
    {
        var cancellationToken = new CancellationTokenSource().Token;
        ...
        var request = new RequestAlterarPermissoesUsuario
        {
            Id = _usuarioRemoverPermissoes.Id.ToString(),
            PerfisAcesso = null,
            Permissoes = []
        };
    ...
    public async Task Sucesso_Usuario_Com_Permissao()
    {
        ...
        var request = new RequestAlterarPermissoesUsuario
        {
            Id = _usuarioGanharPermissoes2.Id.ToString(),
            PerfisAcesso = [_perfilAcessoGerente.Nome],
            Permissoes = [_permissaoCadastroEquipes.Nome]
        };
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoPut(_baseUrl, cancellationToken, request, "TokenInvalido");
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoPut(url: _baseUrl, cancellationToken: cancellationToken, request: request, token: token, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoPut(url: _baseUrl, cancellationToken: cancellationToken, request: request, token: token, appToken: "app-token-invalido");
