...
namespace Integracao.Test.Equipe.Pegar;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class PegarEquipeTest : FfkApiClassFixture
{
    ...
    public async Task Sucesso_Usuario_Com_Permissao_Pegando_Equipe_Da_Propria_Organizacao()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlEquipe}/{_equipeNova.Id}", cancellationToken, token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));

        var dadosDaResposta = await HttpResponseUtil.PegarDadosDaResposta(response);

        AssertDadosDaRespostaComEquipe(dadosDaResposta, _equipeNova);
    }
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlEquipe}/{_equipeNova.Id}", cancellationToken, "tokenInvalid");
    ...
    public async Task Erro_Sem_Token()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlEquipe}/{_equipeNova.Id}", cancellationToken);
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlEquipe}/{_equipeNova.Id}", cancellationToken, token, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlEquipe}/{_equipeNova.Id}", cancellationToken, token, appToken: "app-token-invalido");
