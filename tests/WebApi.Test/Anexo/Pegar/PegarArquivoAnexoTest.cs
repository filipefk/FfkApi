...
namespace Integracao.Test.Anexo.Pegar;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class PegarArquivoAnexoTest : FfkApiClassFixture
{
    private readonly string _baseUrlAnexo = "anexo";
    private readonly string _baseUrlAnexoArquivo = "anexo/arquivo";
    ...
    private readonly FfkApi.Domain.Entities.Anexo _anexoNovo;

    public PegarArquivoAnexoTest()
    {
        ...
        _anexoNovo = (FfkApi.Domain.Entities.Anexo)entidades["AnexoNovo"];
    }

    private async Task<FfkApi.Domain.Entities.Anexo> CadastrarNovoAnexo()
    {
    ...
    public async Task Sucesso_Administrador()
    {
        ...
        var anexo = await CadastrarNovoAnexo();

        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{anexo.Id}", cancellationToken, token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));

        Assert.That(response.Content.Headers.ContentDisposition, Is.Not.Null);
        Assert.That(response.Content.Headers.ContentDisposition!.FileNameStar, Is.EqualTo(anexo.NomeArquivo));

        Assert.That(response.Content.Headers.ContentType, Is.Not.Null);
        Assert.That(response.Content.Headers.ContentType!.MediaType, Is.EqualTo(anexo.MimeType));

        Assert.That(response.Content.Headers.ContentLength, Is.EqualTo(anexo.TamanhoBytes));
    }
    ...
    public async Task Erro_Anexo_Nao_Encontrado()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{Guid.NewGuid()}", cancellationToken, token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NotFound));

        var erros = await HttpResponseUtil.PegarMensagensDeErro(response);

        var mensagemEsperada = MessagesException.GetString("ANEXO_NAO_ENCONTRADO");
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{_anexoNovo.Id}", cancellationToken, "tokenInvalid");
    ...
    public async Task Erro_Id_Invalido(string id)
    {
        ...
        var response = await HttpHelper.DoGet(url: $"{_baseUrlAnexoArquivo}/{id}", cancellationToken: cancellationToken, token: token);
    ...
    public async Task Erro_Sem_Token()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{_anexoNovo.Id}", cancellationToken);
    ...
    public async Task Erro_Token_De_Usuario_Nao_Encontrado()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{_anexoNovo.Id}", cancellationToken, token);
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{_anexoNovo.Id}", cancellationToken, token, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlAnexoArquivo}/{_anexoNovo.Id}", cancellationToken, token, appToken: "app-token-invalido");
