...
namespace Integracao.Test.Anexo.Pesquisar;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class PesquisarAnexoTest : FfkApiClassFixture
{
    private readonly string _baseUrlPesquisar = "anexo/pesquisar";
    private readonly string _queryTest = "Filter=NomeArquivo eq '{nome-arquivo}' and TamanhoBytes eq {tamanho}";
    ...
    private string AjustaQuery(FfkApi.Domain.Entities.Anexo anexo)
    {
        return _queryTest.Replace("{nome-arquivo}", _anexoNovo.NomeArquivo).Replace("{tamanho}", _anexoNovo.TamanhoBytes.ToString());
    }
    ...
    public async Task Sucesso_Administrador()
    {
        ...
        var query = AjustaQuery(_anexoNovo);
        ...
        var arrayAnexos = dadosDaResposta.RootElement.GetProperty("resultados").EnumerateArray();
        Assert.That(arrayAnexos.Count(), Is.GreaterThanOrEqualTo(1));

        var primeiroAnexo = arrayAnexos.FirstOrDefault();

        var id = primeiroAnexo.GetProperty("id").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(id));
        Assert.That(id, Is.EqualTo(_anexoNovo.Id.ToString()));

        var nome = primeiroAnexo.GetProperty("nome").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(nome));
        Assert.That(nome, Is.EqualTo(_anexoNovo.Nome));

        var descricao = primeiroAnexo.GetProperty("descricao").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(descricao));
        Assert.That(descricao, Is.EqualTo(_anexoNovo.Descricao));

        var nomeArquivo = primeiroAnexo.GetProperty("nomeArquivo").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(nomeArquivo));
        Assert.That(nomeArquivo, Is.EqualTo(_anexoNovo.NomeArquivo));

        var tamanhoBytes = primeiroAnexo.GetProperty("tamanhoBytes").GetInt64();
        Assert.That(tamanhoBytes, Is.Not.Null);
        Assert.That(tamanhoBytes, Is.EqualTo(_anexoNovo.TamanhoBytes));
    ...
    public async Task Erro_Anexo_Nao_Encontrado()
    {
        ...
        var query = "Filter=NomeArquivo eq ''";
