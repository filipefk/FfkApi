...
namespace Integracao.Test.Feed.CadastrarEmLote;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class CadastrarFeedEmLoteTest : FfkApiClassFixture
{
    ...
    public CadastrarFeedEmLoteTest()
    {
    ...
    public async Task Sucesso_Total()
    {
        ...
        {
            feed.Organizacao = _organizacaoFfkApi.Nome;
        }

        request.Feeds[0].VisibilidadeUsuarios = [_usuarioSemPerfilNemPermissao.Email];
        request.Feeds[0].VisibilidadeEquipes = [_equipeNova.Nome];

        request.Feeds[1].VisibilidadeUsuarios = [];
        request.Feeds[1].VisibilidadeEquipes = [_equipeNova.Nome];
        ...
        for (int i = 3; i < request.Feeds.Count; i++)
        {
            ...
            request.Feeds[i].VisibilidadeEquipes = [_equipeNova.Nome];
        }
