...
namespace Aceitacao.Test.Indisponibilidade.Pesquisar;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class E2EPesquisarIndisponibilidadeTest : E2EClassFixture
{
    ...
    public async Task Sucesso_Administrador_Pesquisando_Para_Outra_Organizacao()
    {
        ...
        var query = AjustaQuery(indisponibilidadeOrganizacaoNova);
        ...
        AssertJsonElementComIndisponibilidade(primeiraIndisponibilidade, indisponibilidadeOrganizacaoNova);
    ...
    public async Task Sucesso_Usuario_Com_Permissao_Pesquisando_Para_Mesma_Organizacao()
    {
        ...
        var query = AjustaQuery(indisponibilidade);
        ...
        AssertJsonElementComIndisponibilidade(primeiraIndisponibilidade, indisponibilidade);
    ...
    public async Task Sucesso_Usuario_Sem_Permissao_Pesquisando_Para_Mesma_Organizacao()
    {
        ...
        var usuarioSemPerfilNemPermissao = await CadastroHelper.CadastrarNovoUsuario(
            nomeOrganizacao: _usuarioAdministrador.Organizacao.Nome,
            ativar: true);

        var usuarioNovo = await CadastroHelper.CadastrarNovoUsuario(
            nomeOrganizacao: usuarioSemPerfilNemPermissao.Organizacao.Nome,
            ativar: true);
    ...
    public async Task Erro_Usuario_Com_Permissao_Pesquisando_Para_Outra_Organizacao()
    {
        ...
        var organizacaoNova = await CadastroHelper.CadastrarNovaOrganizacao();
        var usuarioNovoDeOutraOrganizacao = await CadastroHelper.CadastrarNovoUsuario(
            nomeOrganizacao: organizacaoNova.Nome,
            ativar: true);

        var indisponibilidadeDeOutraOrganizacao = await CadastroHelper.CadastrarNovaIndisponibilidade(
            usuario: usuarioNovoDeOutraOrganizacao,
            dataInicial: DateOnly.FromDateTime(DateTime.Now),
            dataFinal: DateOnly.FromDateTime(DateTime.Now));

        var token = GeradorTokenUsuarioBuilder.Build().Gerar(usuarioPermissaoCadastroIndisponibilidades.Id);

        var query = AjustaQuery(indisponibilidadeDeOutraOrganizacao);
