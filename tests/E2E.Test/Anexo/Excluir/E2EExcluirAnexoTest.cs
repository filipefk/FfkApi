...
namespace Aceitacao.Test.Anexo.Excluir;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class E2EExcluirAnexoTest : E2EClassFixture
{
    ...
    public async Task Sucesso_Administrador()
    {
        ...
        var response = await HttpHelper.DoGet(url: $"{_baseUrlAnexo}/{anexo.Id}", token: token, cancellationToken: cancellationToken);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));

        response = await HttpHelper.DoDelete(url: $"{_baseUrlAnexo}/{anexo.Id}", token: token, cancellationToken: cancellationToken);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NoContent));

        response = await HttpHelper.DoGet(url: $"{_baseUrlAnexo}/{anexo.Id}", token: token, cancellationToken: cancellationToken);
    ...
    public async Task Erro_Anexo_Nao_Encontrado()
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlAnexo}/{Guid.NewGuid()}", token: token, cancellationToken: cancellationToken);
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlAnexo}/{Guid.NewGuid()}", cancellationToken, "tokenInvalid");
    ...
    public async Task Erro_Id_Invalido(string id)
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlAnexo}/{id}", cancellationToken: cancellationToken, token: token);
    ...
    public async Task Erro_Sem_Token()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlAnexo}/{Guid.NewGuid()}", cancellationToken);
    ...
    public async Task Erro_Token_De_Usuario_Nao_Encontrado()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlAnexo}/{Guid.NewGuid()}", cancellationToken, token);
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlAnexo}/{Guid.NewGuid()}", token: token, cancellationToken: cancellationToken, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlAnexo}/{Guid.NewGuid()}", token: token, cancellationToken: cancellationToken, appToken: "app-token-invalido");
