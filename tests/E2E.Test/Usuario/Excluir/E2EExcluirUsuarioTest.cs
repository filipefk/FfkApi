...
namespace Aceitacao.Test.Usuario.Excluir;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class E2EExcluirUsuarioTest : E2EClassFixture
{
    ...
    public async Task Sucesso_Administrador()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlUsuario}/{usuario.Id}", cancellationToken, token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NoContent));

        response = await HttpHelper.DoGet($"{_baseUrlUsuario}/{usuario.Id}", cancellationToken, token);
        ...
        Assert.That(status, Is.EqualTo("Excluido"));
    }
    ...
    public async Task Erro_Usuario_Ja_Excluido()
    {
        ...
        response = await HttpHelper.DoDelete(url: $"{_baseUrlUsuario}/{usuario.Id}", cancellationToken: cancellationToken, token: token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.Forbidden));

        var erros = await HttpResponseUtil.PegarMensagensDeErro(response);

        var mensagemEsperada = MessagesException.GetString("USUARIO_JA_EXCLUIDO");
    ...
    public async Task Erro_Auto_Exclusao()
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlUsuario}/{_usuarioAdministrador.Id}", cancellationToken: cancellationToken, token: token);

        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.Forbidden));

        var erros = await HttpResponseUtil.PegarMensagensDeErro(response);

        var mensagemEsperada = MessagesException.GetString("AUTO_EXCLUSAO");
    ...
    public async Task Erro_Usuario_Nao_Encontrado()
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlUsuario}/{Guid.NewGuid()}", cancellationToken: cancellationToken, token: token);
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlUsuario}/{_usuarioAdministrador.Id}", cancellationToken, "tokenInvalid");
    ...
    public async Task Erro_Id_Invalido(string id)
    {
        ...
        var response = await HttpHelper.DoDelete(url: $"{_baseUrlUsuario}/{id}", cancellationToken: cancellationToken, token: token);
    ...
    public async Task Erro_Sem_Token()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlUsuario}/{_usuarioAdministrador.Id}", cancellationToken);
    ...
    public async Task Erro_Token_De_Usuario_Nao_Encontrado()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlUsuario}/{_usuarioAdministrador.Id}", cancellationToken, token);
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlUsuario}/{_usuarioSemPerfilNemPermissao.Id}", cancellationToken, token, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoDelete($"{_baseUrlUsuario}/{_usuarioSemPerfilNemPermissao.Id}", cancellationToken, token, appToken: "app-token-invalido");
