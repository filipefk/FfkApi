...
namespace Aceitacao.Test.Usuario.Pegar;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class E2EPegarUsuarioLogadoTest : E2EClassFixture
{
    ...
    public async Task Sucesso_Administrador()
    {
        ...
        var response = await HttpHelper.DoGet(_baseUrlUsuario, cancellationToken, token);
        ...
        Assert.That(perfisAcesso.ToListString(), Is.EquivalentTo(_usuarioAdministrador.PerfisAcesso.ToListNome()!));

        var permissoes = dadosDaResposta.RootElement.GetProperty("permissoes").EnumerateArray();
        Assert.That(permissoes, Is.Not.Null);
        Assert.That(permissoes.ToListString(), Is.EquivalentTo(_usuarioAdministrador.Permissoes.ToListNome()!));
    }
    ...
    public async Task Sucesso_Usuario_Com_Permissao()
    {
        ...
        var id = dadosDaResposta.RootElement.GetProperty("id").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(id));
        Assert.That(id, Is.EqualTo(_usuarioPermissaoCadastroUsuarios.Id.ToString()));

        var nome = dadosDaResposta.RootElement.GetProperty("nome").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(nome));
        Assert.That(nome, Is.EqualTo(_usuarioPermissaoCadastroUsuarios.Nome));

        var email = dadosDaResposta.RootElement.GetProperty("email").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(email));
        Assert.That(email, Is.EqualTo(_usuarioPermissaoCadastroUsuarios.Email));

        var cpf = dadosDaResposta.RootElement.GetProperty("cpf").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(cpf));
        Assert.That(cpf, Is.EqualTo(_usuarioPermissaoCadastroUsuarios.Cpf));

        var telefone = dadosDaResposta.RootElement.GetProperty("telefone").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(telefone));
        Assert.That(telefone, Is.EqualTo(_usuarioPermissaoCadastroUsuarios.Telefone));

        var status = dadosDaResposta.RootElement.GetProperty("status").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(status));
        Assert.That(status, Is.EqualTo(_usuarioPermissaoCadastroUsuarios.Status.ToString()));
        ...
        var perfisAcesso = dadosDaResposta.RootElement.GetProperty("perfisAcesso").EnumerateArray();
        Assert.That(perfisAcesso, Is.Not.Null);
        Assert.That(perfisAcesso.ToListString(), Is.EquivalentTo(_usuarioPermissaoCadastroUsuarios.PerfisAcesso.ToListNome()!));

        var permissoes = dadosDaResposta.RootElement.GetProperty("permissoes").EnumerateArray();
        Assert.That(permissoes, Is.Not.Null);
        Assert.That(permissoes.ToListString(), Is.EquivalentTo(_usuarioPermissaoCadastroUsuarios.Permissoes.ToListNome()!));
    }
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet(_baseUrlUsuario, cancellationToken, "tokenInvalid");
    ...
    public async Task Erro_Sem_Token()
    {
        ...
        var response = await HttpHelper.DoGet(_baseUrlUsuario, cancellationToken);
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoGet(_baseUrlUsuario, cancellationToken, token, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet(_baseUrlUsuario, cancellationToken, token, appToken: "app-token-invalido");
