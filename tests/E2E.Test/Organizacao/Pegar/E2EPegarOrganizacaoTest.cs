...
namespace Aceitacao.Test.Organizacao.Pegar;

[TestFixture]
[Parallelizable(ParallelScope.All)]
public class E2EPegarOrganizacaoTest : E2EClassFixture
{
    private readonly string _baseUrlOrganizacao = "organizacao";
    private readonly string _baseUrlPesquisar = "organizacao/pesquisar";

    private async Task<FfkApi.Domain.Entities.Organizacao> PegarDadosPrimeiraOrganizacao()
    {
        var cancellationToken = new CancellationTokenSource().Token;

        var token = GeradorTokenUsuarioBuilder.Build().Gerar(_usuarioAdministrador.Id);

        var response = await HttpHelper.DoGet(_baseUrlPesquisar, cancellationToken, token);
    ...
    public async Task Sucesso_Administrador()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlOrganizacao}/{organizacao.Id}", cancellationToken, token);
        ...
        Assert.That(!string.IsNullOrWhiteSpace(id));
        Assert.That(id, Is.EqualTo(organizacao.Id.ToString()));

        var nome = dadosDaResposta.RootElement.GetProperty("nome").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(nome));
        Assert.That(nome, Is.EqualTo(organizacao.Nome));

        var descricao = dadosDaResposta.RootElement.GetProperty("descricao").GetString();
        Assert.That(!string.IsNullOrWhiteSpace(descricao));
        Assert.That(descricao, Is.EqualTo(organizacao.Descricao));
    }
    ...
    public async Task Erro_Organizacao_Nao_Encontrada()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlOrganizacao}/{Guid.NewGuid()}", cancellationToken, token);
    ...
    public async Task Erro_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlOrganizacao}/{Guid.NewGuid()}", cancellationToken, "tokenInvalid");
    ...
    public async Task Erro_Id_Invalido(string id)
    {
        ...
        var response = await HttpHelper.DoGet(url: $"{_baseUrlOrganizacao}/{id}", cancellationToken: cancellationToken, token: token);
    ...
    public async Task Erro_Sem_Token()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlOrganizacao}/{Guid.NewGuid()}", cancellationToken);
    ...
    public async Task Erro_App_Token_Ausente()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlOrganizacao}/{organizacao.Id}", cancellationToken, token, addAppToken: false);
    ...
    public async Task Erro_App_Token_Invalido()
    {
        ...
        var response = await HttpHelper.DoGet($"{_baseUrlOrganizacao}/{organizacao.Id}", cancellationToken, token, appToken: "app-token-invalido");
